...
var GameBoard = React.createClass({ displayName: "GameBoard",
  ...
  render: function () {
    ...
    return /*#__PURE__*/(
      ...
      React.createElement("button", { onClick: this.aboutPopOpen }, "Об игре"), /*#__PURE__*/
      React.createElement(Statistics, { board: this.state.board }), /*#__PURE__*/
      React.createElement(Popup, { shown: this.state.popShown, close: this.aboutPopClose, copy: " Привет! Спасибо, что посмотрели мою игру в шашки. Я знаю, что в названии написано «React Checkers», но здесь используется не тонна React, а только обработка дисплея (это его работа, а?). По сути, React отображает наш массив плат, и большая часть перемещений и обнаружений просто обращается к этому массиву. ИИ построен с использованием ограниченной версии минимаксного алгоритма (см. Http:// Neverstopbuilding.com/minimax, чтобы получить хорошее объяснение того, что это означает), просто это означает, что программа прогнозирует будущее, предполагает, что вы будете играть так, как будто вы делали то же самое и выбираете маршрут, который, по его мнению, приведет к лучшему для себя, если вы также будете играть в «совершенство», и я использую это слово свободно, потому что этот ИИ в настоящее время смотрит только на 3 поворота в будущее. Он использует балльную систему для определения «хороших» и «плохих» вещей, которые могут произойти, например, если он может выиграть в следующие 3 хода, это результат 100 баллов. Если он проиграет в следующие 3 хода, это будет стоить -100 очков, потеря короля или убийство вражеского короля стоит -25 или 25 очков соответственно, а убийство / потеря обычных фигур приносит + -10 очков. Наконец, классифицирует создание собственного нового короля как стоящее 15 очков, что немного лучше, чем убийство 1 оппонента. Перед каждым ходом бот просматривает примерно 1000-1500 возможных фьючерсов. "})));
